// GENERATED FILE — plain-CommonJS build of seed.ts, committed deliberately.
//
// The production runtime image (see apps/api/Dockerfile) intentionally has
...
//   (then prepend this header to the output and replace this file)
//
// Verified equivalent to seed.ts by running both against a fresh database
// and diffing the resulting row counts/admin user — see DEPLOYMENT.md.
"use strict";
...
async function seedTaxRules() {
    ...
    const ewtRuleData = {
        description: 'ILLUSTRATIVE default expanded withholding tax rate. Real deployments must replace this with distinct rule codes per income payment type (e.g. EWT_PROFESSIONAL_FEES, EWT_RENTALS, EWT_GOODS) per BIR RR 2-98 as amended — there is no single correct EWT rate to fall back to.',
        appliesTo: 'income_payments_subject_to_ewt',
        sourceReference: 'BIR Revenue Regulations No. 2-98, as amended — PLACEHOLDER VALUE, NOT VERIFIABLE AS A SINGLE RATE. Replace with per-income-type rule codes before relying on EWT computations.',
        effectiveFrom: EFFECTIVE_FROM,
    };
    ...
    // on for real filings.
    const sssRuleData = {
        description: 'Employee share of SSS contribution: 5% of monthly salary, capped at the ₱35,000 Maximum Salary Credit (₱1,750/month max). Approximation of the official bracketed table — see source reference for known gaps (MSC floor, WISP, exact per-bracket rounding).',
        appliesTo: 'employees',
        sourceReference: 'SSS Circular No. 2024-006 (Dec 19, 2024), effective Jan 1, 2025 — verified via sss.gov.ph, Jul 2026. Approximation: does not model the MSC floor, WISP add-on, or exact ₱500-increment bracket rounding.',
        effectiveFrom: EFFECTIVE_FROM,
    };
    ...
    const philhealthRuleData = {
        description: 'Employee share of PhilHealth premium contribution: 2.5% of monthly basic salary (half of the 5% total premium), capped at ₱2,500/month for salaries at or above ₱100,000. Does not apply the ₱10,000 salary floor (flat ₱250/month minimum) — see source reference.',
        appliesTo: 'employees',
        sourceReference: 'PhilHealth advisory, May 2026: 5% total premium rate confirmed as the final scheduled step under RA 11223 (Universal Health Care Act) — verified via pia.gov.ph, Jul 2026. Approximation: does not model the ₱10,000 salary floor.',
        effectiveFrom: EFFECTIVE_FROM,
    };
    ...
    const pagibigRuleData = {
        description: 'Employee share of Pag-IBIG (HDMF) Fund contribution: 2% of monthly compensation, capped at ₱200/month for salaries at or above the ₱10,000 Maximum Fund Salary. Does not apply the 1% lower rate for compensation ≤ ₱1,500/month — see source reference.',
        appliesTo: 'employees',
        sourceReference: 'HDMF Circular No. 460 (Maximum Fund Salary raised to ₱10,000, effective Feb 2024, unchanged through 2026) — verified Jul 2026. Approximation: does not model the 1% lower-income rate for compensation ≤ ₱1,500/month.',
        effectiveFrom: EFFECTIVE_FROM,
    };
...
async function seedPlatformAdmin() {
    ...
    if (!email || !password) {
        ...
        console.log('Skipping platform admin seed — set SEED_ADMIN_EMAIL and SEED_ADMIN_PASSWORD env vars to create one.');
        return;
    }
