...
**Migrations** run via the Dockerfile's own `CMD` on every boot — `render.yaml` deliberately does **not** set a `dockerCommand` override for `pinoytax-api` (idempotent either way — reports "No pending migrations to apply" once applied), the same "chain into whatever the platform always runs to completion" pattern proven on Railway.

**Do not add a `dockerCommand` override that duplicates this.** An earlier revision of `render.yaml` set `dockerCommand: sh -c "npx prisma migrate deploy && exec supervisord -c /app/docker/supervisord.conf"` — byte-for-byte the same command the Dockerfile's `CMD` already runs — and it broke the deploy: Render passes `dockerCommand` through its own string-to-argv conversion, and that conversion mishandles a compound `&&` shell command inside `sh -c "..."`. The live deploy log showed `sh` receiving the *entire* command text as one literal argument instead of a `-c`/script pair (`sh: npx prisma migrate deploy && exec supervisord -c /app/docker/supervisord.conf: not found`, container exited 127 — never reached the database, so this was never the `CREATE ROLE` permission theory an earlier note here speculated about). The Dockerfile's `CMD` is already the correct JSON exec-form array (`["sh", "-c", "npx prisma migrate deploy && exec supervisord -c /app/docker/supervisord.conf"]`), which Docker parses natively with no string-tokenization ambiguity — exactly the mechanism `pinoytax-web` already relies on successfully below. If the start command ever needs to change, edit the Dockerfile's `CMD`, not `render.yaml`.
